...
- 子要素の選択確定後に不要な`CLEAR_CHILD_PREVIEW`を送信せず、プレビュー用commandの一時的なmessage-port切断が成功済みの選択ステータスをエラー表示で上書きしないようにしました。

## [0.15.2] - 2026-08-23

### Added

- Hierarchyの子要素プルダウンをlistboxへ変更し、候補へhoverまたはkeyboard focusした時点でページ上の虹色枠をプレビューするようにしました。
- Esc、外側クリック、Tabなどで候補選択をキャンセルした場合は、現在固定中の元要素へ虹色枠を戻します。候補クリックまたはEnter / Spaceでのみ実際の選択を確定します。

## [0.15.1] - 2026-08-08

### Added

- `秒後に固定`へ`遅延固定後`プルダウンを追加し、`選択のみ`、Standard JSONコピー・保存、CSS Selectorコピーを固定直後に自動実行できるようにしました。

### Fixed

- Stylesの長いComputed Style値をカード内で折り返し、値だけがカード外へ突き抜けたり、Styles全体に不要な横スクロールが出たりしないようにしました。
