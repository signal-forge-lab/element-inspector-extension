...
- 選択要素、現在表示中のviewport、または現在frameのページ全体を対象としたAI Snapshotの生成
...
ユーザーがコピーまたは保存を実行した場合、選択した情報はユーザー自身のクリップボードまたはローカルファイルへ出力されます。AI Snapshotで`Current viewport`または`Full page`を選択した場合は、選択Elementだけでなく現在frame内の表示範囲またはページ全体の内容が出力対象になります。その後の保管・共有はユーザーの管理下にあります。
