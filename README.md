...
- 通常時は最小360×440pxを維持し、ビューポートがそれ未満の場合だけ8px余白内へ縮小します。
...
`遅延固定後`では、固定した直後に自動実行する処理を選べます。デフォルトは`選択のみ`です。
...
- 最後の子要素
- 子要素一覧から任意の子を選択
- 子要素プルダウンでは候補へマウスを乗せるかキーボードフォーカスを移すだけで、その候補をページ上の虹色枠でプレビューします。確定前にEscまたは外側クリックで閉じた場合は、元の固定要素へ枠を戻します。
...
パネル幅・高さはInspectorを閉じるまで維持し、Storageへ保存しません。画面サイズが変わった場合は、パネルがビューポート外へ出ないようサイズと位置を補正します。
